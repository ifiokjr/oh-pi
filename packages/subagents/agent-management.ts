...
export function formatAgentDetail(agent: AgentConfig): string {
	...
	if (agent.extraFields?.category) {lines.push(`Category: ${agent.extraFields.category}`);}
...
export function handleCreate(params: ManagementParams, ctx: ManagementContext): AgentToolResult<Details> {
	...
	if (isChain) {
		...
		if (missing.length > 0) {warnings.push(`Warning: chain steps reference unknown agents: ${missing.join(", ")}.`);}
...
export function handleUpdate(params: ManagementParams, ctx: ManagementContext): AgentToolResult<Details> {
	...
	if (params.agent) {
		...
		if (updated.name !== oldName) {
			...
			if (refs.length > 0) {warnings.push(`Warning: chains still reference '${oldName}': ${refs.join(", ")}.`);}
		}
		const headline =
...
export function handleDelete(params: ManagementParams, ctx: ManagementContext): AgentToolResult<Details> {
	...
	if (params.agent) {
		...
		if (refs.length > 0) {lines.push(`Warning: chains reference deleted agent '${target.name}': ${refs.join(", ")}.`);}
		return result(lines.join("\n"));
	}
	const targetOrError = resolveTarget(
		"chain",
