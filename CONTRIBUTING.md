...
**Every change must include a changeset.** This is enforced in CI on every push and PR. The only exception is `chore: release` commits generated by knope.

```bash
knope document-change
```

Because this repo uses lockstep versioning and a single knope `[package]`, changeset frontmatter must use **only** `default` as the key — not package names.
...
If you use package names like `@ifi/oh-pi` or `@ifi/oh-pi-extensions` in a changeset frontmatter, knope will ignore that changeset in this repo.
