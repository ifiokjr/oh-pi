...
- [ ] Pin errors and budget-exceeded warnings to a top alert area (prevent them from being buried in
      task lists).
...
- [x] Unify signal text and UI label naming (e.g. display text for `PLANNING_RECOVERY` /
      `BUDGET_EXCEEDED`).
- [x] Complete round-1 theme contrast audit and converge color tier usage (see
      `docs/COLONY-UI-CONTRAST-AUDIT.md`).
