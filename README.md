...
oh-pi auto-detects your environment, guides setup in a modern TUI, and writes `~/.pi/agent/` for you. Already configured? It detects existing files and offers **backup before overwriting**.

## Packages

This is a monorepo. Each package can be installed independently as a pi package or together via the meta-package.
...
Intercepts dangerous commands before execution. Blocks `rm -rf`, `git push --force`, `DROP TABLE`, `chmod 777`, and other destructive operations. Also protects configured paths from modification.

**How it works:** Hooks into the `tool_call` event for `bash`, `edit`, and `write` tools. When a dangerous pattern is detected, it prompts for confirmation or blocks outright.
...
**Shows:** Model name with thinking-level indicator, input/output tokens, accumulated cost, context window %, elapsed time, working directory, and git branch.
...
Automatically backgrounds long-running commands (dev servers, builds, test suites). When a command exceeds a 10-second timeout, it's moved to the background and the agent gets the PID + log file path.

**How it works:** Overrides the built-in `bash` tool. Spawns commands with a timer — if they're still running after 10s, detaches them and writes output to `/tmp/oh-pi-bg-*.log`. Provides a `bg_status` tool for listing, viewing logs, and stopping background processes.
...
**CodexBar-inspired** rate limit and cost monitor. The main feature is showing **provider-level rate limits** — how much of your weekly/session Claude and Codex quota remains, with reset countdowns. Also tracks per-model token usage and session costs.

**How it works:** Probes the `claude` and `codex` CLIs (same approach as [CodexBar](https://github.com/steipete/CodexBar)) to get real rate limit data. Tracks local token usage from `AssistantMessage.usage` on every `turn_end` event. Alerts when session cost crosses $0.50, $1, $2, $5, $10+ thresholds.
...
The `usage_report` tool is LLM-callable — the agent can answer "how much quota do I have left?" directly.
...
Releases are done locally using the release script:
...
This script:
1. Runs `knope release` to bump versions, update CHANGELOG.md, and create git tags
2. Builds all packages
3. Publishes to npm (when ready)
